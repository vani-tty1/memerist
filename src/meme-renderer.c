        ...
        for (l = layers; l != NULL; l = l->next, idx++) {
            ...
            if (layer->type == LAYER_TYPE_IMAGE && layer->pixbuf) {
                ...
                PangoFontDescription *desc2 = layer->font_family
                                                  ? pango_font_description_from_string(layer->font_family)
                                                  : pango_font_description_from_string("Sans Bold");
                pango_font_description_set_absolute_size(desc2, layer->font_size * PANGO_SCALE);
                ...
                GdkPixbuf *text_pb = gdk_pixbuf_get_from_surface(surf, 0, 0, tw + 10, th + 10);
                cairo_destroy(cr);
                cairo_surface_destroy(surf);
                g_object_unref(layout2);

                text_pixbufs[idx] = text_pb;
                gpu_layers[idx].pixbuf = text_pb;
            }
        }
